...
class SchoolHolidayService {
  ...
  async getHolidayInfo(zone, date = new Date()) {
    ...
    const isActiveYesterday = isActiveOnDate(periods, yesterday);
    const isActiveTomorrow = isActiveOnDate(periods, tomorrow);
    const yesterdayLabel = labelOnDate(periods, yesterday);
    const tomorrowLabel = labelOnDate(periods, tomorrow);

    return {
      isActive,
      currentLabel,
      currentType: schoolHolidayTypeFromLabel(currentLabel),
      nextLabel: nextPeriod ? nextPeriod.label : null,
      nextType: nextPeriod ? schoolHolidayTypeFromLabel(nextPeriod.label) : null,
      daysUntilNext,
      daysUntilEnd,
      startsToday,
      ...
      isActiveYesterday,
      yesterdayLabel,
      yesterdayType: schoolHolidayTypeFromLabel(yesterdayLabel),
      isActiveTomorrow,
      tomorrowLabel,
      tomorrowType: schoolHolidayTypeFromLabel(tomorrowLabel),
    };
  }
